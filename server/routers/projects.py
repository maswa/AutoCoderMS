...
async def get_research_docs(name: str):
    ...
    Returns the markdown documentation files generated by the Research Agent
    from the .planning/codebase directory.
    """
    _init_imports()
    _, _, get_project_path, _, _ = _get_registry_functions()
    ...
    # Research docs are stored in .planning/codebase directory
    docs_path = project_dir / ".planning" / "codebase"

    if not docs_path.exists():
        raise HTTPException(
            status_code=404,
            detail="Research documentation not found. Run codebase analysis first."
        )

    # Expected documentation files from Research Agent
    doc_filenames = [
        "STACK.md",
        "ARCHITECTURE.md",
        "STRUCTURE.md",
    ...
    if not docs:
        raise HTTPException(
            status_code=404,
            detail="No research documentation files found in .planning/codebase"
        )

    # Get the modification time of the directory for generated_at timestamp
    try:
        generated_at = docs_path.stat().st_mtime
    except Exception:
        generated_at = 0
